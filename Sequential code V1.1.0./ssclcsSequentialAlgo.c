...
long main(long argc, char const *argv[]){
    ...
    f1 = (long*) calloc((m)*(n)*(l) + 2, (sizeof(long))); // The last  +1 is for case who have i, j or k > -1
    f2 = (long*) calloc((m)*(n)*(l) + 2, (sizeof(long))); // The last  +1 is for case who have i, j or k > -1
    ...
    f1[(n-1)*n*l + ((n-1)*l) + (l - 1) + 2] = LONG_MIN;
    f2[(n-1)*n*l + ((n-1)*l) + (l - 1) + 2] = LONG_MIN;
    for (long i = 0; i < m; i++){
        for (long j = 0; j < n; j++){
            for (long k = 0; k < l; k++){
                long res1 = 0;
