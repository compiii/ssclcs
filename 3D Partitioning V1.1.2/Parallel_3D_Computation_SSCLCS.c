...
void allocations(){
    ...
    f1Tmp = (short*) calloc((m)*(n) + 2, (sizeof(short)));
    f2Tmp = (short*) calloc((m)*(n) + 2, (sizeof(short)));
...
void preliminaryWork(){
    ...
    f1Tmp[(n-1)*n + ((n-1)) + 1] = 0;
    f2Tmp[(n-1)*n + ((n-1)) + 1] = 0;
    f1Tmp[(n-1)*n + ((n-1)) + 2] = SHRT_MIN;
    f2Tmp[(n-1)*n + ((n-1)) + 2] = SHRT_MIN;
}
...
void blockCompute(BlockData blockData){
    ...
    MPI_Type_vector(count, blockLengths, 1, MPI_SHORT, &vectorLin);
	...
	for (long i = blockData.coord.i; i <= lig; i++){
        for (long j = blockData.coord.j; j <= col; j++){
            for (long k = 0; k < depth; k++){
                ...
                // if((rank == maxProcess - 1) && (i == m-1) && (j == n-1) && (k == depth-1)) printf("P_%d.\t f1[%ld] = %d \n computation time = %f \n",
