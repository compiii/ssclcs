...
void blockCompute(BlockData blockData){
    ...
    tmpDouble = MPI_Wtime();
	if((rank != 0) && (blockData.rank == rank - 1)){
		//***************** Receive calculated table cells ****************//
        for (int i = 0; i < blockData.dim.nbrLine; i++){
            MPI_Recv(&f1[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, rank - 1, i + blockData.id, MPI_COMM_WORLD, &status);
		    MPI_Recv(&f2[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, rank - 1, i + m*m + blockData.id, MPI_COMM_WORLD, &status);
        }
	}
	...
	if(blockData.rank == rank){
        ...
        if((rank != maxProcess - 1)){
            ...
            for (int i = 0; i < blockData.dim.nbrLine; i++){
                MPI_Isend(&f1[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, rank + 1, i + blockData.id, MPI_COMM_WORLD, &request);
                MPI_Isend(&f2[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, rank + 1, i + m*m + blockData.id, MPI_COMM_WORLD, &request);
            }
        }
	}
