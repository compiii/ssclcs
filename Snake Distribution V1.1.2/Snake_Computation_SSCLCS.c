...
void blockCompute(BlockData blockData){
    ...
    if(((blockData.id == 0) && (blockData.rank != rank)) || ((blockData.rank != rank) && (blockData.id != maxBlock - 1))){
        tmpDouble = MPI_Wtime();
        for (int i = 0; i < blockData.dim.nbrLine; i++){
            MPI_Recv(&f1[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, blockData.rank, (rank*maxProcess + i) + blockData.id, MPI_COMM_WORLD, &status);
            MPI_Recv(&f2[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, blockData.rank, (rank*maxProcess + i) + m*m + blockData.id, MPI_COMM_WORLD, &status);
        }
	...
	if(blockData.rank == rank){ 
        ...
        if(blockData.id != maxBlock - 1){
                    /****** sends calculated table cells ****************/
            tmpDouble = MPI_Wtime();
            for (int j = 0; j < maxProcess; j++){
                if(j != rank){
                    for (int i = 0; i < blockData.dim.nbrLine; i++){
                        MPI_Isend(&f1[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, j, (j*maxProcess + i) + blockData.id, MPI_COMM_WORLD, &request);
                        MPI_Isend(&f2[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, j, (j*maxProcess + i) + m*m + blockData.id, MPI_COMM_WORLD, &request);
                    }
                }
            }
            comTime += MPI_Wtime() - tmpDouble;
