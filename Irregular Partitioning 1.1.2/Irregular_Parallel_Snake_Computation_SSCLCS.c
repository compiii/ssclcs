...
void allocations(){
    ...
    f1 = (short*) calloc((m)*(n)*(depth) + 2, (sizeof(short)));
    f2 = (short*) calloc((m)*(n)*(depth) + 2, (sizeof(short)));
...
void getData(){
    ...
    for(int i = 0; i < depth; i++){
        fgets(listparts[i].partition, contraintLength+1, DNAConstraint); 
...
void preliminaryWork(){
    ...
    f1[i_j_k2(n-1, n-1, depth - 1) + 2] = SHRT_MIN;
    f2[i_j_k2(n-1, n-1, depth - 1) + 2] = SHRT_MIN;
}
...
void blockCompute(BlockData blockData){
	...
	count = 1;
	blockLengths = blockData.dim.nbrColumn * depth;
	stride = 1; 
	strideCol = depth*m;

	MPI_Type_vector(count, blockLengths, stride, MPI_SHORT, &line);
	MPI_Type_commit(&line);

		//***************** Receive calculated table cells ****************// 
    if(((blockData.id == 0) && (blockData.rank != rank)) || ((blockData.rank != rank) && (blockData.id != indice_Itteration - 1))){
        tmpDouble = MPI_Wtime();
        for (int i = 0; i < blockData.dim.nbrLine; i++){
            MPI_Recv(&f1[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, blockData.rank, (rank*indice_Itteration + i) + blockData.id, MPI_COMM_WORLD, &status);
            MPI_Recv(&f2[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, blockData.rank, (rank*indice_Itteration + i) + m*m + blockData.id, MPI_COMM_WORLD, &status);
        }
        comTime += MPI_Wtime() - tmpDouble;
    }
	if(blockData.rank == rank){
	    //***************** Calcul du bloc de données ... *****************//
		tmpDouble = MPI_Wtime();
		computeBlock(lig, col, blockData);
		calculTime += MPI_Wtime() - tmpDouble;

        if(blockData.id != indice_Itteration - 1){
                    /****** sends calculated table cells ****************/
            tmpDouble = MPI_Wtime();
            for (int j = 0; j < maxProcess; j++){
                if(j != rank){
                    for (int i = 0; i < blockData.dim.nbrLine; i++){
                        MPI_Isend(&f1[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, j, (j*indice_Itteration + i) + blockData.id, MPI_COMM_WORLD, &request);
                        MPI_Isend(&f2[i_j_k2(blockData.i+i,blockData.j,0)], 1, line, j, (j*indice_Itteration + i) + m*m + blockData.id, MPI_COMM_WORLD, &request);
                    }
                }
            }
            comTime += MPI_Wtime() - tmpDouble;
...
Boundary computeLimitBlock(BlockData blockData){
	Boundary bound;
	bound.lig = blockData.i + blockData.dim.nbrLine - 1;
	bound.col = blockData.j + blockData.dim.nbrColumn - 1;
	return bound;
}

void allBlockCompute(){
	int i = 0;
	while(i < indice_Itteration){
		blockCompute(arrayDataSplit[i]);
	    i++;
	}
}

void computeBlock(int lig, int col, BlockData blockData){
	for (long i = blockData.i; i <= lig; i++){
        for (long j = blockData.j; j <= col; j++){
