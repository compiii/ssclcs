...
#include "Irregular_Parallel_Snake_Partition_SSCLCS.c"
#include "Irregular_Parallel_Snake_Computation_SSCLCS.c"
#include "clogger.h"
#include "clogger.c"
#include "config.c"
#include "config.h"
...
{   
    ...
    if (argc == 5){
        ...
        k_frag = atol(argv[4]);
        if((k_frag > 2) || (maxProcess == 2)){
            if(rank == 0) printf("Our program supports 2 maximum fragmentations with 3 processors minimal!\n");
            exit(0);
        }
        // offsetConstraint = atol(argv[4]);
    }else{
        if(rank == 0) printf("Arguments incorrect !\n");
        exit(0);
    }
    ...
    inputInfo.maxFragmentation = k_frag;
    ...
    if(rank == arrayDataSplit[indice_Itteration - 1].rank){
